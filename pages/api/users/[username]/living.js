      ...
      if (username) {
        ...
        if (user) {
          const livingProperty = await Property.findOne({ tenants: { $elemMatch: { user: user._id } } })
            ...
            .populate({ path: 'owner', select: '_id username name surname profilePicture' })

          if (livingProperty) {
            livingProperty.tenants.forEach(tenant => {
              const tenantId = tenant.user._id.toString()
              const tenantDate = livingProperty.tenants.find(t => t.user._id.toString() === tenantId).date.toLocaleDateString()
